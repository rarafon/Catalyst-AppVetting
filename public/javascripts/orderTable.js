  ...
  function orderTable(inTableClass, colmn) {
    ...
    while (switching) {
        ...
        for (i = 1; i < (rows.length - 1); i++) {
            ...
            if (curr == "ascending") {
                if  (((x && x.attributes && x.attributes.fullDate && x.attributes.fullDate.value) || (x && x.innerText && x.innerText.toLowerCase()) || "") >
                     ((y && y.attributes && y.attributes.fullDate && y.attributes.fullDate.value) || (y && y.innerText && y.innerText.toLowerCase()) || "")
                    ) {
                        reverse = true;
                        break;
                }
